#!/bin/bash
# Research prototype runner (2026-09-04), transport-apps gap-2-1 revision.
# Block J: the paint rate (25 Hz) for every shape, on a quiet machine, with the
# block's own opening and closing baselines — the row the design quoted came
# from a block whose baseline logged 25 late pulses.
# Block K: a second client's tap arriving inside another client's burst, and
# the same shape with the read batch bounded.
set -u
...
run () {
	...
	echo "== $tag : $*  loadavg $LOAD_START  tap=${TAP_IN_BURST:-0}" >> $LOG
...
run J baseline
run J ws-thread              --rate 25
run J ws-coalesce            --rate 25
run J ws-batch-read          --rate 25
run J ws-direct              --rate 25
run J baseline

export TAP_IN_BURST=1
run K ws-thread              --burst 128  --burst-period 2.0
run K ws-batch-read          --burst 128  --burst-period 2.0
run K ws-batch-read-capped   --burst 128  --burst-period 2.0 --cap 64
