"""PROTOTYPE (revision, research only). Stock Sequencer, no patterns, across tempi,
under the default epoll loop and under a SelectSelector loop."""
...
async def run(bpm, bars, jitter):
	seq = subsequence.sequencer.Sequencer(output_device_name="NO-SUCH-DEVICE-XYZ-REV2",
	                                      initial_bpm=bpm, spin_wait=True, _jitter_log=jitter)
	await seq.start()
	try:
...
for name, sel in (("default (epoll)", None), ("SelectSelector", selectors.SelectSelector)):
	for bpm in (120, 130, 140, 150, 160, 180):
		m, p95, mx, over, n = go(bpm, 8, sel)
		print(f"{name:16s} {bpm:3d} BPM  median {m:7.4f} ms  p95 {p95:7.4f} ms  max {mx:7.4f} ms  over 1 ms {over}/{n}", flush=True)
