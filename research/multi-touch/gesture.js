 ...
 *
 * Tap semantics follow the Digitakt shape: a press on an EMPTY cell sets it the
 * moment the finger lands (act on press, so the cell lights from pointerdown);
 * a quick press on a SET cell clears it on release; holding either for holdMs
 * opens the edit strip without clearing. The recogniser never asks "is
 * multi-touch available". Every gesture that uses two contacts also has a
 * sequential one-contact form built into the same state machine (hold ->
 * release leaves the edit strip latched), so the page degrades without a mode
 * switch. Capability is observed for diagnostics only. A secondary mouse button
 * (button 2) on a cell opens the strip at once, for hover-capable hosts.
 *
...
(function (root, factory) {
	...
	const DEFAULTS = {
		...
		defaultVel: 100,    // subsequence.constants.velocity.DEFAULT_VELOCITY (velocity.py:8; hit_steps default, pattern_builder.py:681)
	...
	// Grid recogniser.
	//
	// Input: down/move/up/cancel with (pointerId, pointerType, cell {row, col} or null, x, y, t ms, button).
	// Output: a list of actions, each {type, ...}:
	//   {type:"set", cell, on:true}                a press on an empty cell: set at once, from pointerdown
	//   {type:"set", cell, on:false}               a quick release on a set cell: clear, from pointerup
	//   {type:"paint", cell, on}                   drag across a row: each cell entered takes the first cell's new state
	//   {type:"editOpen", cells}                   hold reached (or secondary button): strip opens for the held selection
	...
	//   {type:"cancelled", cell}                   pointercancel: the contact is discarded; whatever was already sent stands
	//
	// The page's state (which cells are on) is supplied through opts.isOn(cell) so the
	// recogniser can decide press and paint semantics and never holds a copy of the grid.
	// ---------------------------------------------------------------------
	function GridGestures (opts) {
		this.o = Object.assign({}, DEFAULTS, opts || {});
		this.isOn = this.o.isOn || (() => false);
		this.pointers = new Map();   // id -> {type, target, x0, y0, t0, moved, role, paintOn, setOnPress, lastCell}
	...
	// button: PointerEvent.button (0 primary, 2 secondary); omitted means primary.
	GridGestures.prototype.down = function (id, type, target, x, y, t, button) {
		const out = [];
		this.cap.down(id, type);
		this.tick(t, out);
		const p = {type, target, x0: x, y0: y, t0: t, moved: false, role: null, paintOn: null, setOnPress: false,
			lastCell: target.kind === "cell" ? target.cell : null};
		...
		// (Push: hold several pads to edit them together) and never sets or clears.
		if (this.edit) {
			p.role = "select";
			this._openEdit(target.cell, id, out);
			return out;
		}
		// Secondary mouse button: the hover-capable host's shortcut to the strip.
		// (contextmenu is suppressed page-side, so the button is free.)
		if (button === 2) {
			p.role = "hold";
			this._openEdit(target.cell, id, out);
			return out;
		}
		// Primary contact on a cell: the hold timer decides between tap, hold and paint.
		const on = this.isOn(target.cell);
		p.role = "pending";
		p.paintOn = !on;
		if (!on) {
			// Act on press. Digitakt: "Place note trigs on the sequencer using the sixteen [TRIG] keys";
			// Push 3: "Pressing a pad in a column will add a note to the corresponding step".
			out.push({type: "set", cell: target.cell, on: true});
			p.setOnPress = true;
		}
		return out;
	};
	...
	GridGestures.prototype.move = function (id, target, x, y, t) {
		...
		if (p.role === "pending" && p.moved) {
			...
			// A setting paint already set its first cell on press; a clearing paint clears it now.
			p.role = "paint";
			if (!p.setOnPress) out.push({type: "paint", cell: p.target.cell, on: p.paintOn});
		}
	...
	GridGestures.prototype.up = function (id, t) {
		...
		if (p.role === "pending") {
			// Short press without movement. An empty cell was set on press and nothing more happens;
			// a set cell is cleared now (Digitakt: "Quickly pressing the [TRIG] key of any of the trigs entered will remove the trig").
			if (!p.setOnPress) out.push({type: "set", cell: p.target.cell, on: false});
	...
	GridGestures.prototype.cancel = function (id, t) {
		...
		// Nothing further is emitted; a set already sent on press stands and is reconciled by the ack.
		const out = [];
		const p = this.pointers.get(id);
		this.cap.up(id);
		if (!p) return out;
