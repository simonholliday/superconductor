Summary: The three questions #2018 raises in prose and records as not yet filed are `needs_input` spikes in project `superintendent` — #2021 the thread-to-loop crossing rule, #2019 the scope of the read-only engine exposures, #2020 the listen port — each carrying its short list, its figures and its citations, each linked to #2018 and to the documents it rests on, and #2018 is amended in nine places so that its question list names all fifty-three. Checking the three changed all of them, and the corrections are on the spikes. The crossing short list as #2018 wrote it names a mechanism that does not exist and omits one the engine already uses: on the real `Sequencer`, a pending flag scheduling one drain per burst left the loop wake-up count equal to the message count in every arm, what actually collapses a burst is batching at the socket read (89 crossings for 712 messages, no pulse over 1 ms in 768, three paired passes), and a third shape — the link thread replacing the value on its own thread and never crossing — measured indistinguishable from its own paired baseline with zero wake-ups, so it is now a column, priced at the cost of #1925's single-writer invariant. Waking the loop per set still stands as the recommendation, on the isolated-tap figure (p99 0.168 ms, no pulse over 1 ms in 1536) rather than on the coalescing #2018 credited it with, and not for bursts: a chord of eight cost 3 to 7 late pulses per 768 in every per-message run, one of them 3.73 ms. The exposure set #2018 proposes widening #1963 to is six items, of which three are already reachable without touching a private name, so the real widening is the pattern's grid, its drum note map and the energy-gate flag, with `cycle_start_pulse` kept in the question because the connect snapshot has no public route to it; asking that corrected set as one question stands as the recommendation, but not on #1465, which sorts values rather than verbs. The port recommendation of 8090 survives two corrections: RFC 6335 reserves use-without-assignment for the Dynamic range and not for these, and 8085 is Bamboo's default, so #2018's fallback rule pointed at the vendor it was meant to escape.
...
**What the short list changed.** #2018 gives three columns: drain at the hooks, wake the loop "coalescing a burst into one wake-up", and the second plus the engine-side selector lever. The coalescing does not exist (measured below), and a shape the engine itself already uses was missing from the list: the link thread can replace the value on its own thread and never cross at all. The list below is the corrected one. The selector lever is not a column of its own, because it modifies B rather than competing with it.
...
Revised after verification, so that what Simon reads is what this document says: #2021's short list now carries the corrected three columns with the off-loop option, the pairing paragraph carries the third pairing, the three focus passes are all reported and the load ranges are the true ones; #2019 carries the corrected line numbers (`composition.py:5970`, `5967` and `5976`), keeps `cycle_start_pulse` in option C for the connect snapshot, says what becomes of #1963 in each column, and no longer rests its recommendation on #1465; #2020 carries the corrected RFC 6335 reading and Bamboo's claim on 8085. #1968's own body gains a paragraph saying that its "both are inside 0.1 s on the LAN" holds only under the crossing rules that apply a panel write promptly, and naming #2021 as the question to answer with it. A comment on #2018 records the three corrections against its body: that its short-list table still labels the middle column as coalescing a burst into one wake-up, which the measurement refutes; that 8085 is Bamboo's default, so its port fallback rule points at the vendor it was meant to escape, and that RFC 6335 is not a licence to take a User Port; and that the exposure bullet's three line numbers were off by one.
...
Subroutine read: #2018 in full and in its pre-amendment copy, #1925, #1926, #1914, #1929, #1963, #1968, #1465 (and #1908's title, through #1465's own record), #2019, #2020, #2021, and the specification for this research.
...
Subroutine items written: #2021, #2019 and #2020 filed as `needs_input` spikes in project `superintendent` and revised after verification; a paragraph added to #1968's body naming #2021 as the question to answer with it; a comment on #2018 recording the three corrections against its body; #2018's body amended.
