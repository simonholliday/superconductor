"""PROTOTYPE, not house style.  Variant adding the batch and off-loop arms.
...
the crossing rule, with an identical arrival process in every arm:
  ...
  direct       the link thread writes the value itself and never crosses onto
               the clock loop (whole-value replacement, the shape #1914 records
               as safe by construction and sequencer.py:790-803 already uses)
...
def main () -> None:
	...
	async def _run () -> None:
		...
		def producer () -> None:
			nonlocal produced, drain_pending, applied
			...
			while not stop_flag.is_set():
				...
				if now < next_at:
					...
					continue
				next_at += interval
				count = burst if pattern == "burst" else 1
				if arm == "direct":
					# No crossing at all: the link thread replaces the value on
					# its own thread.  The clock loop is never woken.
					for _ in range(count):
						produced += 1
						grid[produced % 128] = produced
						applied += 1
						if burst > 1 and pattern == "burst":
							time.sleep(0.001)
					continue
				if arm == "batch":
					# One socket read hands the link thread the whole burst; it
					# queues all of it and crosses onto the clock loop once.
					for _ in range(count):
						produced += 1
						inbound.put(produced)
