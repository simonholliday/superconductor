...
async def main(rate):
	async with pa.async_playwright() as p:
		...
		await page.wait_for_function("() => { const s=document.getElementById('summarized-results'); const e=document.getElementById('result-number'); return s && s.classList.contains('selected') && e && e.textContent.trim().length>0; }", timeout=1500000)
